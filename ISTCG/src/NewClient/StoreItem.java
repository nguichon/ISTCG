...
public class StoreItem extends Canvas {
    ...
    public StoreItem(Composite parent, int style, String id, int cost,
            String name, String decription, StoreUI store) {
        super(parent, style);
        SetCost(cost);
        SetDes(decription);
        SetName(name);
        ...
        storeUI = store;
        BGImage = i_Name + ".png";
        // TODO SetBGImage(): ?? Image Manager? needs size 300x400 (same as card
        // size)
    ...
    public void Render(GC targetGC, Point size) {
        targetGC.drawImage(ImageManager.get().GetImage(BGImage), 0, 0, 300,
                400, 0, 0, size.x, size.y);
